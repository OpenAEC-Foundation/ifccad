# IFCDR JSON mapping language v3

This document normatively defines `ifccad.ifcdr.jsonMapping.v3`. It applies to
mapping documents using that identifier, including
[the IFCDR 0.9.0 mapping](json-mapping-0.9.0.json). The
[meta-schema](json-mapping-meta-schema-v3.json) checks the structure of a mapping
...
Mapping v3 provides no wildcard, array-index or escaped-dot path syntax.
...
The 0.9.0 mapping uses this only for polyline.placement. An absent placement
...
For the 0.9.0 polyline mapping these names are `vertexOffset`, `vertexCount`,
...
In mapping v3, child ranges MUST form a complete contiguous partition of
