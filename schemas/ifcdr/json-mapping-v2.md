# IFCDR JSON mapping language v2

This document normatively defines `ifccad.ifcdr.jsonMapping.v2`. It applies to
mapping documents using that identifier, including
[the IFCDR 0.8.0 mapping](json-mapping-0.8.0.json). The
[meta-schema](json-mapping-meta-schema-v2.json) checks the structure of a mapping
...
Mapping v2 provides no wildcard, array-index or escaped-dot path syntax.
...
The 0.8.0 mapping uses this only for polyline.placement. An absent placement
...
zero. No default is inferred for required record members or scope baseZ.
...
For the 0.8.0 polyline mapping these names are `vertexOffset`, `vertexCount`,
...
In mapping v2, child ranges MUST form a complete contiguous partition of
