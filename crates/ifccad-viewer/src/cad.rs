use crate::{fail, inspect_package, progress, result};
use ifccad::{package::PackageOptions, PackageId};
...
fn convert_document(doc: &CadDocument, output: &Path, mut r: Value) -> Value {
    progress("converting");
  ...
  json!({"handle":handle.to_string(),"kind":e.as_entity().entity_type(),"disposition":disposition,"target":target.map(|id|json!({"resourceId":null,"entityId":id.get().to_string()})),"reasons":losses.iter().flat_map(|d|d.reasons().iter().map(|r|format!("{r:?}"))).collect::<Vec<_>>()})
    ...
    if let Err(e) = out.package().write_directory(output) {
        fail(&mut r, "preparing", "PACKAGE_WRITE_FAILED", e);
        r["conversion"]["entities"] = json!(entities);
        return r;
    }
    progress("validating");
    let package = inspect_package(output);
