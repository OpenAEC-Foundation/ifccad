# Drawing resource contract 0.13.0

This candidate is normative alongside ifccad-overlay-0.13.0.json, drawing core
0.4.0 and IFCDR 0.11.0. Its presence does not imply reader/writer support before
...
an IFCDR resource with format openaec.ifcdr, version 0.11.0 and role drawing.
...
An optional grouped `Drawing.attributes.pointDisplay` defines one drawing-wide
point marker default. Its `form` has a `glyph` (`dot`, `hidden`, `plus`,
`cross`, or `shortLine`) and independent boolean `circle` and `square`
enclosures. Its `size` is one of `defaultFivePercent`, `absolute` with a
positive finite drawing-unit value, or `viewportPercent` with a positive
finite value. An absent setting resolves to dot without enclosures and
`defaultFivePercent`. `defaultFivePercent` is distinct from an explicit
`viewportPercent` value of 5. Point placement may orient the marker; the
setting does not change point geometry or bounds.
...
Viewport-effective style inheritance is specified in the IFCDR 0.11.0 logical
...
fields in drawing core 0.4.0. Unknown values or partial present settings are
