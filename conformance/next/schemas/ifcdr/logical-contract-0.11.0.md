# IFCDR 0.11.0 logical contract

This document, [the logical registry](registry-0.11.0.json), and the applicable
encoding mapping define the IFCDR 0.11.0 contract. The
[JSON reference mapping](json-mapping-0.11.0.json) specifies physical storage.
...
Point, Circle, Arc, Ellipse, EllipseArc and Line columns describe corresponding rows.
PlanarPolyline and SpatialPolyline rows have common entity properties and an
ordered vertex sequence. `entityOrder` associates a
...
`point` is a distinct entity family with the common entity identity, scope,
layer, appearance, visibility and draw-order rules. Its complete `placement`
uses the same `plane.frame` validation as a planar polyline. The origin is the
point's scope-local XYZ position; X and Y orient a possible planar marker and
do not create geometric extent. Point bounds consist of that one position.
No second position field or per-point marker style is defined.

## Circular entities

`circle` stores a plane placement and finite positive radius. Its placement
origin is the centre. X/Y define the curve plane, with X cross Y defining the
normal. `arc` adds finite `startParameter` and signed `sweepParameter` in
radians. Its sweep must satisfy `0 < abs(sweepParameter) < 2π`; a zero or full
turn is invalid. The start is measured from X toward Y, and a negative sweep
traverses toward negative Y. Readers preserve start and sweep values without
normalizing them. Circle and Arc retain their own entity kinds even when an arc
approaches a full turn. Scope bounds enclose actual curve extrema, including
extrema between arc endpoints.

`ellipse` stores a plane placement, finite `semiMajorRadius` and
`semiMinorRadius` with `semiMajorRadius >= semiMinorRadius > 0`. X points along
the semimajor direction and Y along the semiminor direction. Equal radii retain
the Ellipse kind. `ellipseArc` adds the same finite start and signed partial
sweep rules as Arc, including its own kind for an almost full turn. Its
parameterized point is `origin + X * semiMajorRadius * cos(t) + Y *
semiMinorRadius * sin(t)`. Bounds include interior extrema of the actual
elliptic segment.
...
`geometry.finite`: Line endpoints, spatial polyline vertices and block base points
are finite XYZ values. Planar polyline vertices are finite local XY values with
finite bulges. Endpoint Z defaults independently
...
Widths and fit curves have no logical representation in this version.
...
`polyline.vertex-count`: Both polyline families have at least two vertices, whether open
...
polyline is valid. There is no nonzero-area or nonzero-length constraint on
straight segments. A planar polyline stores one bulge per vertex; the bulge of
the last vertex of an open polyline is retained but does not draw a segment.
For a closed polyline it controls the closing segment. A nonzero bulge denotes a
circular arc with signed `tan(includedAngle/4)` in the placement's X/Y plane.
The endpoints of an active bulged segment must be distinct. SpatialPolyline has
no placement or bulges; its vertices are scope-local XYZ coordinates.
...
Otherwise finite ordered XYZ bounds enclose all its evaluated geometry: point
origins, line endpoints, placed planar polyline vertices and curved interiors,
spatial polyline vertices and recursively instanced contents. An
...
The [0.11.0 mapping](json-mapping-0.11.0.json) applies those forms to this
