...
#[test]
fn overlay_0_11_selects_block_candidate_and_preserves_extensions() {
    ...
    let validator = jsonschema::draft202012::options()
        ...
        .build(&load_schema("ifccad-overlay-0.11.0.json"))
        .unwrap();
    for source in [
        json!({"content":{}}),
        json!({"uri":"drawing.json", "checksum":format!("sha256:{}", "a".repeat(64))}),
    ] {
        for (version, valid) in [("0.9.0", true), ("0.8.0", false)] {
            ...
            document["extension"] = json!({"arbitrary":"retained"});
            document["data"] = json!([
                {"path":"r", "type":"openaec:DrawingRepresentation", "attributes":{"resource":descriptor}},
                {"path":"x", "type":"vendor:Extension", "attributes":{"x":42}}]);
            assert_eq!(validator.is_valid(&document), valid);
        }
    }
}
